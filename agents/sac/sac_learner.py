...
from distributions.tanh_normal import TanhNormal
...
class SACLearner(Agent):
    ...
    def create(
        ...
        **kwargs
        ...
        # Actor Network
        action_base_cls = partial(
            MLP, hidden_dims=hidden_dims, activate_final=True, use_pnorm=use_pnorm
        )
        actor_def = TanhNormal(action_base_cls, action_dim)
        ...
        critic_base_cls = partial(
            ...
            dropout_rate=critic_dropout_rate, use_layer_norm=critic_layer_norm,
            use_pnorm=use_pnorm
        )
        critic_cls = partial(StateActionValue, base_cls=critic_base_cls)
        critic_def = Ensemble(net_cls=critic_cls, num=num_qs)
        ...
        target_critic_def = Ensemble(net_cls=critic_cls, num=num_qs)
    ...
    def update_actor(self, batch) -> Tuple[Agent, Dict[str, float]]:
    ...
    def update_critic(self, batch) -> Tuple[TrainState, Dict[str, float]]:
